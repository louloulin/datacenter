...
class ProfilerTestRunner {
    ...
    private fun runOptimizedTest(
        ...
        // 4. 使用内存预分配器
        val memoryPreallocator = MemoryPreallocator(64 * eventCount, 128)
        ...
        val handler = EventHandler<ProfiledEvent> { event, _, _ ->
            try {
                ...
                if (event.data.isNotEmpty()) {
                    ...
                    // 使用预分配内存
                    val buffer = memoryPreallocator.allocate(event.data.size)
                    System.arraycopy(event.data, 0, buffer, 0, event.data.size)
                    memoryPreallocator.release(buffer)
                }
