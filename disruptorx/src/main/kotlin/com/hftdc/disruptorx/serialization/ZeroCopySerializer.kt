...
class ZeroCopySerializer {
        ...
        override fun read(context: DeserializationContext): Array<T> {
            ...
            for (i in 0 until size) {
                val element = when (componentType) {
                    Int::class.javaObjectType, Int::class.javaPrimitiveType -> 
                        (elementHandler as IntTypeHandler).read(context) as T
                    Long::class.javaObjectType, Long::class.javaPrimitiveType -> 
                        (elementHandler as LongTypeHandler).read(context) as T
                    Double::class.javaObjectType, Double::class.javaPrimitiveType -> 
                        (elementHandler as DoubleTypeHandler).read(context) as T
                    Boolean::class.javaObjectType, Boolean::class.javaPrimitiveType -> 
                        (elementHandler as BooleanTypeHandler).read(context) as T
                    String::class.java -> 
    ...
    inner class ObjectTypeHandler<T>(private val type: Class<*>) : DeltaTypeHandler<T> {
        ...
        override fun write(context: SerializationContext, value: T) {
            // 检查引用
            val valueAsAny = value as Any
            if (context.objectReferences.containsKey(valueAsAny)) {
                context.buffer.putInt(-context.objectReferences[valueAsAny]!!)
                return
            }
            
            // 存储引用
            context.objectReferences[valueAsAny] = context.currentPosition
        ...
        override fun read(context: DeserializationContext): T {
            ...
            // 存储引用
            context.objectReferences[context.currentPosition] = instance as Any
        ...
        override fun readDelta(context: DeserializationContext, baseline: T): T {
            ...
            // 创建实例（从基准复制）
            @Suppress("UNCHECKED_CAST")
            val instance = type.getDeclaredConstructor().newInstance() as T
