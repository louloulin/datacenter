...
echo -e "${YELLOW}生成总体分析报告...${NC}"
{
  ...
  find benchmark-results -maxdepth 2 -type d -name "*2025*" | sort | while read dir; do
    date=$(basename $dir)
    csv_file=$(find $dir -name "performance_data.csv" | head -1)
    if [ -n "$csv_file" ]; then
      throughput=$(grep -v timestamp $csv_file | awk -F, '{sum+=$2} END {print sum/NR}')
      p50=$(grep -v timestamp $csv_file | awk -F, '{sum+=$3} END {print sum/NR}')
      p99=$(grep -v timestamp $csv_file | awk -F, '{sum+=$4} END {print sum/NR}')
      p999=$(grep -v timestamp $csv_file | awk -F, '{sum+=$5} END {print sum/NR}')
